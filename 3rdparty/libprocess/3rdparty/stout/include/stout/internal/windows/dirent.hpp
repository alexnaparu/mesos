...
inline DIR* opendir(const char* path)
{
  ...
  directory->d_name =
    (char*) malloc(pathSize + strlen(dirSeparatorAndWildcard) + 1);
...
inline bool _openDirStream(DIR* directory)
{
  ...
  // NOTE: This copy operation is safe because both `cFileName` and `d_name`
  // are arrays of `MAX_PATH` size. See[1].
  //
