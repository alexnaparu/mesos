...
inline Try<Bytes> size(const std::string& path = "/")
{
  ...
  if (!GetDiskFreeSpaceEx(realPath.get().c_str(), &freeBytesAvailable,
    ...
    return WindowsError(
        "Error invoking `GetDiskFreeSpaceEx` on '" +
        path + "'");
  }
...
inline Try<double> usage(const std::string& path = "/")
{
  ...
  if (!GetDiskFreeSpaceEx(
      realPath.get().c_str(), &freeBytesAvailable,
      &totalNumberOfBytes, &totalNumberOfFreeBytes)) {
    return WindowsError(
        "Error invoking `GetDiskFreeSpaceEx` on '" +
        path + "'");
...
{
  return internal::windows::createSymbolicLink(link, original);
}
...
inline Try<std::list<std::string>> list(const std::string& pattern)
{
  ...
  do {
    const std::string currentFile(findData.cFileName);
