        ...
        pid_t(const lambda::function<int()>&)>>& _clone)
{
  ...
  switch (in.mode) {
    case Subprocess::IO::FD: {
      ...
      if (stdinHandle[0] == INVALID_HANDLE_VALUE) {
        // No handles to cleanup, just exit.
    ...
    case Subprocess::IO::PATH: {
      ...
      // `FILE_SHARE_READ` to make the handle read/only (as `stdin` should be),
      // but allow others to read from the same file.
  ...
  switch (out.mode) {
    ...
    case Subprocess::IO::PATH: {
      ...
      if (stdoutHandle[1] == INVALID_HANDLE_VALUE) {
        internal::close(stdinHandle, stdoutHandle, stderrHandle);
        return WindowsError("Failed to open '" + out.path.get() + "'");
      }
      break;
    }
    default:
  ...
  switch (err.mode) {
    ...
    case Subprocess::IO::PATH: {
      // Get a handle to the `stderr` file. Use `GENERIC_WRITE` to make the
      // handle writeable (as `stderr` should be) and do not allow others to
      // share the file.
      ...
      if (stderrHandle[1] == INVALID_HANDLE_VALUE) {
        internal::close(stdinHandle, stdoutHandle, stderrHandle);
        return WindowsError("Failed to open '" + err.path.get() + "'");
      }
      break;
    }
    default:
  ...
  if (!pid.isSome()) {
    internal::close(stdinHandle, stdoutHandle, stderrHandle);
    return Error("Could not launch child process" +
        (pid.isError() ? ": " + pid.error() : ""));
  }

  Subprocess process;
  process.data->pid = pid.get();

  // Close some of the handles that are created by this function. For pipes,
  // we close the child ends and store the parent ends (see code below).
  ::CloseHandle(stdinHandle[0]);
  ::CloseHandle(stdoutHandle[1]);
  ::CloseHandle(stderrHandle[1]);
